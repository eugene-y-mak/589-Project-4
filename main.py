...
if DATA == 0:
    ...
    MASTER_DATASET.columns = MASTER_DATASET.columns.map(str)
    # DROP USELESS COLUMN
    MASTER_DATASET = MASTER_DATASET.drop("Name", axis=1)
    K = 10
    HIDDEN_LAYER_STRUCTURE = [16]
    ALPHA = 1
    EPSILON = 10e-4
    ...
    CSV = 'finaldatasets/loan.csv'
    NAME = "Loan"
    LABEL_HEADER = "Loan_Status"
    CATEGORICALS = ["Gender", "Married", "Dependents", "Education", "Self_Employed", "Credit_History", "Property_Area"]
    MASTER_DATASET = pd.read_csv(CSV)
    MASTER_DATASET.columns = MASTER_DATASET.columns.map(str)
    # DROP USELESS COLUMN
    MASTER_DATASET = MASTER_DATASET.drop("Loan_ID", axis=1)
    K = 10
    HIDDEN_LAYER_STRUCTURE = [16]
    ALPHA = 1
    EPSILON = 10e-4
    ...
    CSV = 'finaldatasets/parkinsons.csv'
    NAME = "Parkinsons"
...
def main():
    print(NAME)
    # normalized_labels = helpers.normalize_dataset(MASTER_DATASET[LABEL_HEADER])
#  normalized_df.columns = normalized_df.columns.map(str)
    #possible_class_labels = helpers.get_attribute_values(normalized_labels, LABEL_HEADER)
    possible_class_labels = np.unique(MASTER_DATASET[LABEL_HEADER])
    # one hot encode entire dataset
    OHE_df = helpers.encode_attribute(MASTER_DATASET, LABEL_HEADER)

    # OHE categoricals
    for category in CATEGORICALS:
        OHE_df = helpers.encode_attribute(OHE_df, category)

    # normalize data
    normalized_OHE_df = helpers.normalize_dataset(OHE_df)
    normalized_OHE_df.columns = normalized_OHE_df.columns.map(str)
